# Retarget adjustments

Use the six **Retarget adjustments** in the **Retarget** tab to correct a consistent pose difference between the tracked actor and one avatar. They add small pose offsets to the retargeted arms, legs, and shoulders. 

| Control | Use | GIF |
| --- | --- | --- |
| **Upper-arm spread** | Move both upper arms farther from or nearer to the torso. | |
| **Forearm preroll** | Adjust the twist of both forearms. | |
| **Upper-arm twist** | Adjust the twist of both upper arms. | |
| **Upper-leg spread** | Move both upper legs farther apart or nearer together. | |
| **Shoulder height** | Adjust shoulder height. | |
| **Shoulder shift** | Adjust shoulder position. | |

Notice that retarget adjustments might be partially overidden by [Spatial hands](spatial-hands.md)'s IK.

